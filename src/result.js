...
export default class Result {
  constructor(doc, html, fastbootInfo) {
    ...
    (fastbootInfo.response ||= {}).statusCode ||= 200;
  ...
  html() {
    ...
    if (statusCode === 204) {
      ...
      this._html = '<body><!-- VITE_EMBER_SSR__BODY --></body>';
      this._head = '';
      this._body = '';
  ...
  chunks() {
    return insertIntoIndexHTML(
      ...
      if (!head || !body) {
        throw new Error(
          'Could not identify head and body of the document! Make sure the document is well formed.'
        );
      }
...
async function insertIntoIndexHTML(
  ...
  html = html.replace(/<!-- VITE_EMBER_SSR_(HEAD|BODY) -->/g, function(match, tag) {
  ...
  if (head && !isHeadReplaced) {
    missingTag('<!-- VITE_EMBER_SSR_HEAD -->');
  }
  if (body && !isBodyReplaced) {
    missingTag('<!-- VITE_EMBER_SSR_BODY -->');
  }
